...
public class Map_data : MonoBehaviour
{
    ...
    //読み込んだマップを保存するとこ
    int[,] map;
    //マップデータ（不動の部分）を渡す、すごい重い
    public int[] Take_map_data(){
        int[] submap = new int[Maxsize_y * Maxsize_x];
        for(int i = 0; i < Maxsize_y * Maxsize_x; i++){
            submap[i] = map[map_num, i];
        }
        return submap;
    }
    int[] L_x;
    ...
    Vector3[] m_instance_scale;//各細胞の大きさを保存する
    ...
    void Awake(){
        ...
        int num = L_x[map_num] * L_y[map_num];
        m_instance_t = new Vector3[num];
        m_instance_scale = new Vector3[num];
        m_instance_color = new Color[num];
        m_instance_uv = new Vector4[num];
        for (int i = 0; i < num; ++i){
            m_instance_t[i] = new Vector3(i % L_x[map_num] - L_x[map_num] / 2.0f, -i / L_x[map_num] + L_y[map_num] / 2.0f, 0);
            switch (map[map_num, i]){
                case 1:
                    m_instance_color[i] = new Color(1.0f, 1.0f, 1.0f, 1.0f);
                    break;
                case 2:
                    break;
                default:
